...
>
> 1. **Backend bug — search by phone number 500s.** `getFilteredOrders` treats any all-digit term as an `orderNumber` and passes `Number(term)` to Prisma as an `int4`; a 10-digit phone overflows Postgres integer range (`Value out of range for the type`). "Search by … phone" — advertised in the placeholder — is broken for essentially every real number. Pinned by **TC-262** with `test.fail()` so it flips loudly when fixed. Fix idea: only match `orderNumber` when the term is ≤ 9 digits / fits int4.
