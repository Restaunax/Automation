...
export const createAdminRestaurantsPage = (page: Page) => {
  ...
  const goto = async () => {
    await page.goto("/admin?tab=restaurants", { waitUntil: "domcontentloaded" });
    await page.getByRole("heading", { name: /restaurants/i }).waitFor({ state: "visible", timeout: 15_000 });
  };

  const assertPageLoaded = () =>
    expect(page.getByRole("heading", { name: /restaurants/i })).toBeVisible({ timeout: 15_000 });

  const assertTableColumnVisible = (columnName: string) =>
    expect(page.getByRole("columnheader", { name: columnName })).toBeVisible({ timeout: 10_000 });
