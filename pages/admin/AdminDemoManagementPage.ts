...
export const createAdminDemoManagementPage = (page: Page) => {
  ...
  const assertSideSheetOpen = (text: string) =>
    expect(page.locator(".MuiDrawer-paper").filter({ hasText: text })).toBeVisible({ timeout: 10_000 });

  const assertDialogOpen = (text: string) =>
    expect(page.locator('[role="dialog"]').filter({ hasText: text })).toBeVisible({ timeout: 10_000 });

  const closeSideSheet = async () => {
    await page.locator(".MuiDrawer-paper").getByRole("button", { name: "Close" }).click();
    await page.locator(".MuiDrawer-paper").waitFor({ state: "hidden", timeout: 5_000 });
  };

  const closeDialog = async () => {
    await page.locator('[role="dialog"]').getByRole("button", { name: /cancel|close/i }).first().click();
