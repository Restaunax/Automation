...
export const createOwnerCouponPage = (page: Page) => {
  ...
  const navigateToCreateCoupon = async () => {
    ...
    const couponsAccordion = drawer().getByRole("button", { name: /coupons/i }).first();
    ...
    await page.getByPlaceholder("SUMMER2025").waitFor({ state: "visible", timeout: 15_000 });
  };
  ...
  const createCouponButton = () => page.getByRole("button", { name: "Create Coupon" });
