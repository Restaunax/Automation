...
export const createOwnerPublishPage = (page: Page) => {
  const goto = async (restaurantId: string) => {
    await page.goto(
      `/restaurant/restaurantId/${restaurantId}/publish`,
      { waitUntil: "domcontentloaded" }
    );
    await page.getByRole("heading", { name: /publish/i }).waitFor({ state: "visible", timeout: 15_000 });
  };
  ...
  const assertChecklistItemVisible = (label: string) =>
    expect(page.getByText(label, { exact: true })).toBeVisible({ timeout: 10_000 });
