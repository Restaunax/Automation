...
export const createOwnerTaxPage = (page: Page) => {
  const goto = async (restaurantId: string) => {
    ...
    await page
      .getByPlaceholder("e.g., 7.5")
      .waitFor({ state: "visible", timeout: 15_000 });
  };

  const taxRateInput = () => page.getByPlaceholder("e.g., 7.5");
  const saveButton = () =>
    page.getByRole("button", { name: "Save Tax Settings" });
  const successToast = () =>
    page.getByText("Tax settings updated successfully!");
  ...
  const assertFormVisible = () =>
    expect(taxRateInput()).toBeVisible({ timeout: 10_000 });
