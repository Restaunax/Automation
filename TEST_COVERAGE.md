...
| Orders — order detail view                                | TC-90                             | ✅ Read-only; an order is API-seeded in beforeAll so a row always exists                                                         |
| Orders — customer order reaches owner (E2E)               | TC-127                            | ✅ Seeds a uniquely-named order via the public API; owner searches, opens it, `detailOrderId` confirms identity; cancelled after |
| Orders — owner accepts (confirms) an order                | TC-128                            | ✅ Owner clicks "Mark as Confirmed" in the detail view; PENDING→CONFIRMED verified at the API source of truth; cancelled after   |
...
| Customer places order → Staff sees it → Staff accepts   | ✅ TC-100 (POS, API level) — seeded order appears in the tablet's live feed and is driven PENDING → … → PICKED_UP. Placement via API seed; real customer-UI placement is TC-26.                                                                          |
| Customer places order → Owner views it in dashboard     | ✅ TC-127 — seeds a uniquely-named order via the public API, then the owner searches for it, opens it, and the detail URL's `detailOrderId` confirms it's that exact order (cancelled in cleanup). TC-90 remains the "any order's detail renders" check. |
