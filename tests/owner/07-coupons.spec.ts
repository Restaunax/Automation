...
import { createOwnerCouponPage } from "../../pages/owner/OwnerCouponPage";
...
test.describe("Owner — Coupons", () => {
  ...
  test("TC-30: owner can navigate to the Create Coupon form", async ({ ownerPage }) => {
    ...
    await allure.step("Expand Coupons section and click Create Coupon", async () => {
  ...
  test("TC-31: owner can fill and submit a new coupon", async ({ ownerPage }) => {
    await allure.description(
    ...
    await allure.step("Fill coupon details", async () => {
      await couponPage.fillCouponForm(couponCode, "10", "2026-07-01", "2026-12-31");
