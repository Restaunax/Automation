...
**Status:** ✅ Passing (TC-262 is a deliberate expected-failure — see below)
...
| **TC-262** | Searching by the customer's 10-digit **phone number** should return their order. **Marked `test.fail()`** — currently the backend 500s (see below).                                                                                                                                                                                            | The search placeholder promises phone search.                                                                                         |

**Real product bug surfaced by TC-262 (2026-08-15):** the list endpoint (`getFilteredOrders`) treats any all-digit search term as a daily order number and hands `Number(term)` to Prisma as an `int4`. A 10-digit phone number overflows Postgres integer range → `500 "Value out of range for the type"`. So "search by phone" is broken for essentially every real phone number. The test is kept red-as-expected with `test.fail()`; when the backend guards the order-number branch, the test will report "unexpected pass" and the marker should be removed.
...
| TC-262          | Search by customer phone number finds the order                                                                                                                                                                                                                                                                                                                 | Owner                 | ⚠️ Expected failure (`test.fail`) — backend 500s on 10-digit numeric search (int4 overflow); flips when fixed                             |
