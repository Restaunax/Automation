...
| 🌐 API-Level      | No UI — direct backend calls       | TC-65, TC-66, TC-68, TC-69, TC-79, TC-80, TC-255 → TC-261 (`api-orders.spec.ts`), TC-263 → TC-287 (`api-menu.spec.ts`), TC-323, TC-324, TC-325 → TC-350 (`api-deals.spec.ts`)                                                                         |
...
**Status:** ✅ Passing (53/53 in the combined run, incl. the pre-existing TC-86/87/195..197). Ships with **eleven `test.fail()` pins** for RestauNax bugs found by the audit (TC-334, 335b, 336, 341c, 343, 347..350, 358) — they turn red on the nightly the day the fix reaches QA; flip them to plain tests then.
...
| **TC-334** 🔴        | pin — an 11th ACTIVE create at the cap must be refused (today 201).                                                                                                                                             | Cap bypass.                                                            |
| **TC-335 / 335b** 🔴 | `active-count` shape; PATCH → ACTIVE at 10/10 refused with `MAX_ACTIVE_DEALS_REACHED` (plain); pin — `PUT {status:ACTIVE}` at the cap must be refused (today 200).                                              | The 10-active rule.                                                    |
| **TC-336** 🔴        | pin — `PUT` must re-apply create's validation (price 0 / "9am" accepted today).                                                                                                                                 | A 0 price flows into the charge.                                       |
...
| **TC-341** 🔴        | Bulk: `[]` → 400, two → 201 with counts; pin — `aiGenerated:false` must be honoured (`\|\| true` today).                                                                                                        | AI Generator's bulk save.                                              |
| **TC-342**           | Public `GET /ai/questions` is static with the four question ids.                                                                                                                                                | Generator step 0, no AI call.                                          |
| **TC-343** 🔴        | pin — the engine must not price a % coupon on top of a deal (`COUPON_RULES_AND_FREE_DELIVERY.md`: "the engine never prices both" — only wind blocks it client-side today).                                      | Money rule enforced server-side, not per client.                       |
...
| **TC-347 – 350** 🔴  | pins — another owner must NOT read our deals/stats/menu-picker/deal, deactivate, edit, create-into or delete (today 200/201 — verified live on QA 2026-08-17).                                                  | Security — same class as menu #601.                                    |
...
| **TC-358** 🔴 | pin — searching from page 2 must show the matching row (page index isn't reset on filter today → empty table).                                                                                                                                                                 |
...
**Real product findings from this batch (2026-08-18) — all recorded in `docs/DEALS_TAB_TEST_STRATEGY.md` §1 and pinned:** (1) authenticated IDOR on **every** `/api/deals` route (TC-347..350; reads verified live on QA); (2) `MAX_ACTIVE_DEALS` bypassable via create and `PUT` (TC-334, 335b); (3) `PUT /:dealId` has no validation — price 0 accepted (TC-336); (4) the pricing engine stacks a coupon on a deal although the rules doc says it never does — only template-wind blocks it client-side (TC-343); (5) Manage Deals pagination isn't reset on filter (TC-358); (6) `bulkCreateDeals` forces `aiGenerated: true` (TC-341); note-only: template-wind `DealsGridBlock` reads the wrong shape and never renders prices, `OrderDeal` snapshots are client-supplied (stats can be corrupted, not the charge), and the confirmation page greets a returning phone number by the **stored** customer name rather than the name typed at checkout (TC-371 uses a fresh phone).
...
| TC-325 → TC-350 | Deals API contract: create math + qty-1 split, validation, list projections, patch, status, hard delete, public /active + /validate windows, cap (🔴 TC-334/335b), 🔴 PUT validation, /quote charge + upcharge + rejections, 🔴 coupon ⊥ deal, stats, bulk (🔴 aiGenerated), AI questions, chain scope, 401s, 🔴 IDOR TC-347..350 (`api-deals.spec.ts`)         | Owner (throwaway tenant) | ✅ Passing · 8 🔴 pins                                                                                                                    |
| TC-351 → TC-364 | Manage Deals table (rows, search, filter, sort, expand, toggle, delete, 🔴 pagination TC-358, cap banners), Create/Edit form (validation, live math, create, edit), Deal Analytics, AI Generator smoke (`11-deals.spec.ts`)                                                                                                                                     | Owner                    | ✅ Passing · 1 🔴 pin                                                                                                                     |
