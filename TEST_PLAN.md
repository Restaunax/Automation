...
| Dual pricing v2 — POS / API contract              | `pos/09-dual-pricing.spec.ts`                                                                                                                         | Own throwaway tenant (the menu conversion rewrites every stored price). Admin-only enrollment + card markup (≤5%), enable refused until a markup exists, mutual exclusion with the fee pass-through; one-time menu conversion (12.95→13.40, 3.00→3.11 at 3.5%) with `cashPrice` pairs on the menu and 409 on re-run; tablet settings `dualPricing` contract with four notices; register cash order priced at the cash tier (pre-tax discount, tax on the cash base); mismatched / card / split claims refused; untouched check settled whole in cash via `applyCashDiscount` (collects exactly the cash total), partly paid check refused; public /details leaks no program flags; editing a cash-priced order re-derives the discount at the markup (qty 2→3: 1.12→1.68, tax on the new cash base). Backend #683 on QA; green 2026-08-29 (TC-484..493). TC-497: compliance attestations are admin-stamped booleans (`dualPricingMenuAttested` → `…AttestedAt`), owner claims stripped, raw timestamps ignored (restaunax #686 on QA; green 2026-08-29). TC-498: the conversion is reversible — REVERT refused while dual pricing is on, then divides every price back by the recorded markup (13.40→12.95, 3.11→3.00; menu serves no `cashPrice` while off), second REVERT 400, CONVERT runs again (restaunax #687 on QA; green 2026-08-29).                                                                                                                                                         |
| Dual pricing v2 — online stays card-only          | `customer/09-dual-pricing.spec.ts`                                                                                                                    | TC-495: with the shared restaurant dual-priced (restored in finally), the wind checkout shows the single card price — no cash wording, no fee line. Backend #683 on QA; green 2026-08-29.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Dual pricing v2 — dashboard UI                    | `dashboard/owner/dual-pricing.spec.ts`                                                                                                                | TC-494: owner Store Settings → Order Settings — the Dual Pricing toggle is disabled (with the "contact RestauNax" warning) until an admin sets the markup, then offered with the 3.5% summary; Convert menu… previews the CR/CA table with the confirm gated behind the acknowledge checkbox (never confirmed — one-way stamp); Price list dialog opens. TC-496: admin Restaurant Management → Edit → Update Restaurant Info renders the enrollment switch, `#dual-pricing-card-markup`, legal note and both actions. Flags snapshotted + restored in finally.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
