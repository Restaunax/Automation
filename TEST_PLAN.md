...
| Deals — follow-ups                         | Flip the 🔴 pins when RestauNax fixes land (TC-334/335b cap, TC-336 PUT validation, TC-341 aiGenerated, TC-343 coupon ⊥ deal in the engine, TC-347..350 IDOR, TC-358 pagination); frontend testids for the Deals components (POMs use accessible names/ids today); wind `DealsGridBlock` shape fix (note only) |
