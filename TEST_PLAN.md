...
- **POS / tablet API harness** — largely built now: `POST /api/tablet/login` +
  order-status transitions (TC-100), and the staff-session / register-session
  chain (`setOwnerPosPin` → `tabletStaffSignIn` → `openRegisterSessionPos`)
  shipped with the open-checks suite (TC-372..383). Remaining: capability-gated
  flows beyond MANAGER self-auth (void/comp/discount approvals, handover/close,
  cash drops).
