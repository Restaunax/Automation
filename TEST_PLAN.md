...
│   │   │   ├── supply-shop.spec.ts           # ✅ real — Supply Shop admin: design queue + preflight (BLOCK on wrong size), Fulfil dialog (final price, out-of-estimate warning, comp), place-on-behalf, COMP → IN_PRODUCTION + gift-card batch + CSV export, finance memo (TC-456..468)
...
│   │   │   ├── 20-supply-shop.spec.ts        # ✅ real — Print Shop (owner): product hidden until gift cards on, estimate range + place with NO charge, designStarted email + CTA, proof changes/approve (409 on stale), admin finalise → Awaiting payment + Pay link, cancel, on-behalf order (TC-446..455)
│   │   │   ├── api-supply-shop.spec.ts       # ✅ real — owner supply-shop API contract: tier math + 25% spread, X-Restaurant-Id tenancy, gift-card gate on search, order list strips internals, proof/cancel refusals (TC-478..483)
