...
| **Emails excluded by default** | Demo, invite, password-reset, sign-up and gift-card tests are tagged `@email` and **excluded from the default run** — they send real mail into a quota-limited Mailtrap inbox. Run them deliberately with `npm run test:email` / `test:demo`. |
