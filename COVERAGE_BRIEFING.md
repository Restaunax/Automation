...
| **Employee**            | ⚠️ (4 tests)          | Tax settings, create-restaurant-for-client. Skip without `EMPLOYEE_*` creds — not a failure                                                                 | menu publish                                                |
...
| 🔴 Security    | **Order create trusts client-supplied money fields** — a `total: 0` order is marked paid with no Stripe charge; a guest can order a real item for $0.                                              | Needs server-side price recompute — backend-team owned                                  |
