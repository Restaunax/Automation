...
| 🟡 Backend bug | **Editing any coupon 500s** — form sent `value` as a string, Prisma expects a Float.                                                                                                               | ✅ Fixed — RestauNax **#481** (numeric fields coerced on form + `updateCoupon`); TC-92 now passing |
...
|                            |                                                                                                                                                                                                                                                                                                                                                                    |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Skipped ≠ broken**       | Many tests skip on purpose when their credentials/environment aren't set (`EMPLOYEE_*`, `TEMPLATE_WIND_URL`, `MAILPIT_BASE_URL`). A skip is a gate, not a failure.                                                                                                                                                                                                 |
| **Emails run by default**  | Demo, invite, password-reset and sign-up tests are tagged `@email` and **run in the default suite** — they send real mail to QA's self-hosted, unmetered Mailpit inbox (readable at https://mail.qa.restaunax.com). `npm run test:email` / `test:demo` just select them. (Gift-card purchase tests are `test.fixme` — blocked by Stripe Radar; see TEST_COVERAGE.) |
